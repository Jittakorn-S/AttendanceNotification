...
namespace AttendanceNotification
{
    public partial class Form1 : Form
    {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            streamFile.Close();
            //string[] GetData = System.IO.File.ReadAllLines(@"C:\Users\user446\Desktop\att\test.csv");
            ...
            foreach (string EachlinesData in GetData)
            {
                ...
                {
                    if (EachlinesData.Contains("DSI") && EachlinesData.Contains(GetDate))
                    {
            ...
            {
                CountDSI = listofID.Distinct().Count();
                int ResultCountDD = MaxDSI - CountDSI;
                if (listofID.Distinct().Count() == MaxDSI)
                {
                    lineNotify("ประจำวันที่: " + DateNowMM + "\n" + "ทำแบบสำรวจครบแล้ว");
                    Application.Exit();
                }
                else if (ResultCountDD < MaxDSI)
                {
            ...
            {
                int ResultCountDD = MaxDSI - listofID.Count;
                if (listofID.Count == MaxDSI)
                {
                    lineNotify("ประจำวันที่: " + DateNowMM + "\n" + "ทำแบบสำรวจครบแล้ว");
                    Application.Exit();
                }
